...
本页记录 `1.1.3` 在 Windows 上的可重复验收结果。验收页面使用与发布包相同的 `src/theme.css` 和内置 WebP，不用单独手绘一张效果图，因此布局改动会直接反映到这些场景中。

## 结论

- 规则白点叠层已移除；默认背景使用 `cover`，在超宽和恢复窗口里不再出现图片边缘接缝。
- 应用菜单栏使用全长半透明表面；任务标题使用 44 px 圆角玻璃条，只保留约三分之一屏宽、最大 `680px` 并单独靠右。
