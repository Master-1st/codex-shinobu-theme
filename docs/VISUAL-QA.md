...
本页记录 `1.1.2` 在 Windows 上的可重复验收结果。验收页面使用与发布包相同的 `src/theme.css` 和内置 WebP，不用单独手绘一张效果图，因此布局改动会直接反映到这些场景中。

## 结论

- 规则白点叠层已移除；默认背景使用 `cover`，在超宽和恢复窗口里不再出现图片边缘接缝。
- 应用菜单栏使用全长半透明表面；任务标题使用 44 px 圆角玻璃条并单独靠右。
...
| 场景 | 尺寸 | 验收门槛 | 结果 |
|---|---:|---|---|
| 普通对话 | 1440×900 | 应用栏 `0/1440px`；回复与输入框 `430/1290px` 居中；任务标题 `543/1403px` 靠右 | 通过 |
| 恢复窗口 + 覆盖侧栏 | 1963×1188 | 侧栏右缘 `330px`；回复与输入框 `716/1576px` 居中；任务标题 `1079/1939px` 靠右 | 通过 |
| 分屏 + 双输入框 | 2048×1229 | 左侧回复与输入框 `317/913px`；右侧输入框 `1491/2011px`；任务标题 `1415/2011px` 靠右 | 通过 |
| 长输出 | 900×800 | 回复与输入框 `230/875px` 居中对齐；内容底边 `662px`，输入框顶边 `672px` | 通过 |
| 窄窗口 | 600×900 | 侧栏隐藏；回复与输入框 `25/575px` 居中；任务标题 `33/583px` 靠右，无页面溢出 | 通过 |
...
![居中玻璃对话与靠右任务标题验收](visual-qa/centered-glass-1963x1188.png)
